...
reauth(){ cd "$APP_DIR"; PYTHONPATH="$SRC_DIR" "$PY" "$SRC_DIR/bootstrap_session.py"; }
...
doctor() {
  ...
  [ -f "$APP_DIR/radar.db" ] && ok "运行数据库: $APP_DIR/radar.db" || warn "数据库尚未生成"
  [ -f "$APP_DIR/sessions/tg_radar_admin.session" ] && ok "Admin Session 已存在" || warn "缺少 admin session"
  [ -f "$APP_DIR/sessions/tg_radar_core.session" ] && ok "Core Session 已存在" || warn "缺少 core session"
...
menu() {
  ...
  printf "%b\n" "${DIM}Root clean · src-based layout · Core/Admin split · SQLite WAL${C0}"
  line
  cat <<'MENU'
1) 写入 / 刷新 systemd 服务
2) 启动服务
