...
class AdminApp:
    ...
    async def run(self) -> None:
        ...
        async with TelegramClient(str(self.config.admin_session), self.config.api_id, self.config.api_hash) as client:
            ...
            self.db.log_event("INFO", "ADMIN", f"admin service started v{__version__}")
            ...
            self.logger.info("admin service stopping")
    ...
    async def dispatch(self, event: events.NewMessage.Event, command: str, args: str) -> None:
        ...
        if command == "help":
            await self.safe_reply(
                event,
                f"""⚙️ <b>TG-Radar Plan C 管理菜单</b>

<code>{prefix}ping</code> - 心跳测试
<code>{prefix}status</code> - 系统大屏
<code>{prefix}log 30</code> - 查看最近 30 条操作日志
